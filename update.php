...
//		return $checked_data; 
// lines will need to be commented in the fb_check_for_plugin_update function as well.
...
add_filter('plugins_api', 'fb_plugin_api_call', 10, 3);
...
function fb_plugin_api_call($def, $action, $args) {
	...
	if (is_wp_error($request)) {
		$res = new WP_Error('plugins_api_failed', __('An Unexpected HTTP Error occurred during the API request.</p> <p><a href="?" 
onclick="document.location.reload(); return false;">Try again</a>'), $request->get_error_message());
	} else {
